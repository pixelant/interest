...
class CrudHandler implements HandlerInterface
{
    ...
    public function createRecord(
        ...
        $event = $this->eventDispatcher->dispatch(GeneralUtility::makeInstance(BeforeDataImportingEvent::class, $importData, $remoteId, $tableName));
        $importData = $event->getImportData();

        if (array_key_exists('returnSuccessResponse', $importData) && $importData['returnSuccessResponse'] === true) {
            return $responseFactory->createSuccessResponse(
                [
                    'status' => 'success',
                    'data' => [
